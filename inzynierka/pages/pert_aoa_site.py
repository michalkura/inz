...
def graph():
    return rx.vstack(
            ...
            rx.box(
                rx.vstack(
                    ...
                    rx.form(
                        rx.hstack(
                            rx.select(
                                pert_aoa_site.nodes_list, placeholder="Select predecessor node.", size="1",
                                name='predecessor'
                            ),
                            rx.select(
                                pert_aoa_site.nodes_list, placeholder="Select successor node.", size="1",
                                name='successor'
                            ),
                            rx.input(
            ...
            rx.box(
                rx.vstack(
                    ...
                    rx.form(
                        rx.hstack(
                            rx.select(
                                pert_aoa_site.nodes_list, placeholder="Select node to delete.", size="1",
                                name='node_del'
                            ),
                            rx.button(
            ...
            rx.box(
                rx.vstack(
                    ...
                    rx.form(
                        rx.hstack(
                            rx.select(
                                pert_aoa_site.edges_list, placeholder="Select edge to delete.", size="1",
                                name='edge_del'
                            ),
                            rx.button(
        ...
        # dataframe
        rx.data_table(
            data=pert_aoa_site.cpm_dataframe,
            pagination=True,
            search=True,
        ),
        rx.divider(),
