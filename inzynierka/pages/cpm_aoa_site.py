...
class cpm_aoa_site(rx.State):
    ...
    def check_layout_error(self, layout: str = None):
        ...
        if not self.G.is_graph_drawable(layout):
    ...
    def handle_edges_submit(self, new_edges: dict):
        ...
        layout = self.selected_layout_real
        if not self.G.is_edge_addable(predecessor, successor, edge_time, layout):
            self.set_default_layout()
    ...
    async def handle_upload(self, files: list[rx.UploadFile]):
        ...
        for index, row in df.iterrows():
            H.add_edge(str(row['predecessor']), str(row['successor']), float(row['time']))
                    ...
                    rx.form(
                        rx.hstack(
                            ...
                            rx.input(
                                ...
                                name='edge_time', type="number", step="any", min=0),
                            rx.button(
